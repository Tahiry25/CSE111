...
def pressure_gain_from_water_height(height):
    Pa = (WATER_DENSITY * EARTH_ACCELERATION_OF_GRAVITY * height)/1000
    return Pa
...
def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):
    loss = (0 - friction_factor * pipe_length * WATER_DENSITY * fluid_velocity**2) / (2000*pipe_diameter)
    return loss

# calculate the water pressure lost because of fittings such as 45° and 90° bends that are in a pipeline.
def pressure_loss_from_fittings(fluid_velocity, quantity_fittings):
    lost_pressure = (0 - (0.04 * WATER_DENSITY * fluid_velocity**2 * quantity_fittings)) / 2000
    return lost_pressure

# calculate and return the Reynolds number for a pipe with water flowing through it.
def reynolds_number(hydraulic_diameter, fluid_velocity):
    pdv = WATER_DENSITY * hydraulic_diameter * fluid_velocity
    return pdv / WATER_DYNAMIC_VISCOSITY
...
def pressure_loss_from_pipe_reduction(larger_diameter, fluid_velocity, reynolds_number, smaller_diameter):
    ...
    p = (0 - Kn * WATER_DENSITY * fluid_velocity**2) / 2000
    return p
